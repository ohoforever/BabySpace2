...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return bool|string
	 */
	private function http_get($url,$retry=3){
	    $this->log('请求URL:'.$url);
		$oCurl = curl_init();
		...
		if(intval($aStatus["http_code"])==200){
            ...
            return $this->http_get($url,--$retry);
		}
	}
	...
	private function http_post($url,$param,$post_file=false,$retry=3){
		...
		if(intval($aStatus["http_code"])==200){
            ...
            return $this->http_post($url,$param,$post_file,--$retry);
		}
	}
	...
	protected function setCache($cachename,$value,$expired){
		...
		$value = [$value,time(),$expired];
		$yac = new Yac('wechat_');
	    return $yac->set($cachename, $value);
...
// 		return false;
	...
	protected function getCache($cachename){
		...
		if(time() - intval($data[1]) >= intval($data[2])){
		    $yac->delete($cachename);
		    return false;
		}
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        if ($result)
        {
            ...
            M('t_wechat')->update(['access_token'=>$this->access_token,'token_ctime'=>time(),'expires_in'=>intval($json['expires_in'])],['appid'=>$appid]);
            return $this->access_token;
        }
     ...
     * @return string
     */
	static function json_encode($arr,$options=JSON_UNESCAPED_UNICODE) {
