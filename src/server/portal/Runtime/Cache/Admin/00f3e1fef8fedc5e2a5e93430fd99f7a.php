...
<div class="tabbable">
    ...
    <div class="tab-content no-border padding-24">
		...
		<?php $_result=parse_config_attr($model['field_group']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><div id="tab<?php echo ($key); ?>" class="tab-pane <?php if(($key) == "1"): ?>active<?php endif; ?> tab<?php echo ($key); ?>">
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; $field['name'] = strtolower($field['name']); ?>
                <?php if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-group cf">
                    <label class="col-xs-12 col-sm-2 control-label no-padding-right"><?php echo ($field['title']); ?></label>
                    <div class="col-xs-12 col-sm-6">
                        <?php switch($field["type"]): case "num": ?><input type="text" class="width-100" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                            <?php case "string": ?><input type="text" class="width-100" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                            <?php case "textarea": ?><textarea name="<?php echo ($field["name"]); ?>" class="form-control"><?php echo ($data[$field['name']]); ?></textarea><?php break;?>
                            <?php case "date": ?><input type="text" name="<?php echo ($field["name"]); ?>" class="width-100 date" value="<?php echo (date('Y-m-d',$data[$field['name']])); ?>" placeholder="请选择日期" /><?php break;?>
                            <?php case "datetime": ?><input type="text" name="<?php echo ($field["name"]); ?>" class="width-100 time" value="<?php echo (date('Y-m-d H:i',$data[$field['name']])); ?>" placeholder="请选择时间" /><?php break;?>
                            ...
                            <?php case "radio": $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label>
                                        <input type="radio" class="ace" value="<?php echo ($key); ?>" name="<?php echo ($field["name"]); ?>" <?php if(($data[$field['name']]) == $key): ?>checked="checked"<?php endif; ?> />
                            ...
                            <?php case "checkbox": $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label>
										<input type="checkbox" class="ace" name="<?php echo ($field["name"]); ?>[]" value="<?php echo ($key); ?>"<?php if(in_array(($key), is_array($data[$field['name']])?$data[$field['name']]:explode(',',$data[$field['name']]))): ?>checked="checked"<?php endif; ?> />
										<span class="lbl"> <?php echo ($vo); ?>&nbsp;</span> 
									</label><?php endforeach; endif; else: echo "" ;endif; break;?>
                            <?php case "editor": ?><textarea name="<?php echo ($field["name"]); ?>"><?php echo ($data[$field['name']]); ?></textarea>
                            ...
                            <?php case "picture": ?><div class="controls">
									...
									<?php if(!empty($data[$field['name']])): ?><div class="upload-pre-item"><img width="120" src="<?php echo (get_cover($data[$field['name']],'path')); ?>"/></div><?php endif; ?>
									</div>
								</div>
								<script type="text/javascript">
								<?php  $thumb = array(); foreach (explode("\r\n", $field['extra']) as $k=>$v){ if(empty($v)){ continue; } $thumb[] = "'thumb[$k]':'$v'"; } ?>
								//上传图片
							    /* 初始化上传插件 */
                            ...
                            <?php default: ?>
                            <input type="text" class="width-100" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php endswitch;?>
                    </div>
