    ...
    <div class="tab-content no-border padding-24">
        <div class="tab-pane active">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><div class="form-group">
                      <label class="col-xs-12 col-sm-2 control-label no-padding-right" for="nickname"><?php echo ($config["title"]); ?></label>
                      <div class="col-xs-12 col-sm-5">
                          <?php switch($config["type"]): case "0": ?><span class="input-icon block input-icon-right">
	                              <input type="text" class="width-100" id="nickname" value="<?php echo ($config["value"]); ?>" name="config[<?php echo ($config["name"]); ?>]">
	                              <i class="icon icon-info-sign"></i>
	                          </span><?php break;?>
				              <?php case "1": ?><span class="input-icon block input-icon-right">
                                  <input type="text" class="width-100" id="nickname" value="<?php echo ($config["value"]); ?>" name="config[<?php echo ($config["name"]); ?>]">
                                  <i class="icon icon-info-sign"></i>
                              </span><?php break;?>
				              <?php case "2": ?><textarea class="autosize-transition span12 form-control" rows="" cols="40" name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea><?php break;?>
				              <?php case "3": ?><textarea class="autosize-transition span12 form-control" rows="5" cols="40" name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea><?php break;?>
				              ...
				              <?php case "5": ?><!--增加富文本和非明文-->
                                   <textarea id="<?php echo $config['name']?>" name="config[<?php echo ($config["name"]); ?>]" style="width:auto;"><?php echo ($config["value"]); ?></textarea>
                                    <?php echo hook('adminArticleEdit', array('id'=>$config['name'],'name'=>"config[".$config['name']."]",'value'=>$config['value'])); break;?>
                    	      <?php case "6": ?><input type="password" class="width-100"  name="config[<?php echo ($config["name"]); ?>]" value="<?php echo ($config["value"]); ?>"><?php break; endswitch;?>
                      </div>
                      <div class="help-block col-xs-12 col-sm-reset inline"><?php echo ($config["remark"]); ?></div>
                </div><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php
 echo ace_srbtn(); echo ace_form_close() ?>
        </div>
    </div>
</div>
